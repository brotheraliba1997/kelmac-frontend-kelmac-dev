...
export default function BasicInfo({ course = dummyCourse }: { course?: Course }) {
    ...
    const total =
        ...
        course.discount +
        (course.regularFee + course.certificationFee + course.examinationFee - course.discount) * (course.taxPercent / 100) -
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span className="ml-2 w-5 h-5 flex items-center justify-center rounded-full bg-blue-100">
                                        <svg
                                            className={`w-3 h-3 text-blue-900 transform transition-transform duration-300 ${isOpen ? "rotate-180" : ""
                                                }`}
                                            fill="none"
                                            stroke="currentColor"
                                            strokeWidth="2"
                                    ...
                                    <div className="absolute top-full right-0 bg-white rounded-lg shadow-xl text-black w-64 overflow-hidden animate-fadeIn z-50">
                                        ...
                                        {sessions.map((session, idx) => (
                                            <button
                                                ...
                                                className={`block w-full text-left px-4 py-3 hover:bg-blue-50 transition ${selectedSession === session ? "bg-blue-100 font-medium" : ""
                                                    }`}
                                            >
                                                {session}
                                            </button>
                ...
                <div className="max-w-6xl mx-auto mt-8 flex flex-col lg:flex-row">
                    ...
                    <div className="w-full lg:w-1/3 flex flex-col items-center">
                        <div className="w-full bg-gray-100 p-6 rounded-xl shadow flex flex-col">
                            <h3 className="text-lg font-semibold mb-4 text-black">Course Fee Summary</h3>

                            <div className="space-y-2">
                                <div className="flex justify-between">
                                    <span className="text-black">Regular Fee:</span>
                                    <span className="text-black">${course.regularFee.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-black">Certifications Fee:</span>
                                    <span className="text-black">${course.certificationFee.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-black">Examinations Fee:</span>
                                    <span className="text-black">${course.examinationFee.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-black">Discount:</span>
                                    <span className="text-black">-${course.discount.toFixed(2)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-black">Tax ({course.taxPercent}%):</span>
                                    <span className="text-green-500">
                                        ${((course.regularFee + course.certificationFee + course.examinationFee - course.discount) * (course.taxPercent / 100)).toFixed(2)}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-black">Coupon Applied:</span>
                                    <span className="text-black">-${course.couponDiscount.toFixed(2)}</span>
                                </div>
